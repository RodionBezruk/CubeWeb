					...
					if($this->_redirectaddr)
					{
						...
						{						
							if(!preg_match("|^".$URI_PARTS["scheme"].":
								$this->_redirectaddr = $this->_expandlinks($this->_redirectaddr,$URI_PARTS["scheme"].":
							if(preg_match("|^http:
							{
								$this->_redirectdepth++;
								$this->lastredirectaddr=$this->_redirectaddr;
	...
	function _httprequest($url,$fp,$URI,$http_method,$content_type="",$body="")
	{
		...
		while($currentHeader = fgets($fp,$this->_maxlinelen))
		{
			...
			if(preg_match("/^(Location:|URI:)/i",$currentHeader))
			{
				...
				{
					$this->_redirectaddr = $URI_PARTS["scheme"].":
        			if(!empty($this->host_port))
		...
		if(($this->_framedepth < $this->maxframes) && preg_match_all("'<frame\s+.*src[\s]*=[\'\"]?([^\'\"\>]+)'i",$results,$match))
		{
			...
			for($x=0; $x<count($match[1]); $x++)
				$this->_frameurls[] = $this->_expandlinks($match[1][$x],$URI_PARTS["scheme"].":
		}
