	...
	this.display=function(){
		...
		el.setAppearance=function(){
			...
			if(this.files.length || this.folders.length){
				...
				if(this.files.length)html+=this.files.length+' file'; 
				if(this.files.length>1)html+='s';
				if(this.folders.length)html+='<br/>'+this.folders.length+' folders'; 
			}
		...
		el.pasteContents=function(){
			...
			if(this.folders.length)kfm.alert('paste of folders is not complete'); 
			this.clearContents();
		};
		kfm_addContextMenu(el,function(e){
			...
			{ 
				plugins.push(['$("kfm_widget_clipboard_container").clearContents()','clear clipboard']); 
				plugins.push(['$("kfm_widget_clipboard_container").pasteContents()','paste clipboard contents']); 
			}
...
kdnd_addDropHandler('widget_clipboard','.kfm_dir_name',function(e){
	...
	var dir_over=parseInt($E('.kfm_directory_link',e.targetElement).node_id);
		...
		links.push(['x_kfm_moveFiles(['+e.sourceElement.files.join(',')+'],'+dir_over+',function(e){if($type(e)=="string")return alert("error: could not move file[s]");kfm_removeFilesFromView(['+selectedFiles.join(',')+'])});kfm_selectNone()','move files',0,!kfm_vars.permissions.file.mv]); 
