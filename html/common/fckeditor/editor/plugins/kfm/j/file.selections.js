...
function kfm_chooseFile(){
	...
	if(kfm_vars.files.return_id_to_cms){
		...
		x_kfm_getFileUrls(selectedFiles,function(urls){
			...
			if(selectedFiles.length==1&&File_getInstance(selectedFiles[0]).width)window.opener.SetUrl(urls[0].replace(/([^:]\/)\
			else{
...
function kfm_selection_dragFinish(e){
	...
	setTimeout('window.dragType=0;',1); 
...
function kfm_shiftFileSelectionUD(dir){
	...
	if(selectedFiles.length){
		if(topOffset==$('kfm_file_icon_'+na[ns-1]).offsetTop)return; 
