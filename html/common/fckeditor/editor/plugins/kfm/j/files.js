...
function kfm_deleteFiles(files){
	var names=[],m='';
	 var dfiles=[]; 
	 for(var j=0;j<files.length;j++)dfiles.push(files[j]);
	if(dfiles.length>10){
...
function kfm_refreshFiles(res){
	if(!res.files)return;
	$j('#folder_info').text(res.files.length+(res.files.length==1?' file':' files')); 
	kdnd_addDropHandler('kfm_file','.kfm_directory_link',function(e){
		...
		if(kfm_vars.files.drags_move_or_copy == 1)eval(mv); 
		else if(kfm_vars.files.drags_move_or_copy == 2)eval(cp); 
		else{
			var links=[];
