...
class kfmImage extends kfmFile{
	...
	function setThumbnail($width=64,$height=64){
		...
		if($r){
			$id=$r['id'];
			if(!file_exists(WORKPATH.'thumbs/'.$id))$this->createThumb($width,$height,$id); 
		}
		else{
		...
		$this->thumb_id=$id;
		$this->thumb_path=str_replace('
		if(!file_exists($this->thumb_path)){
			copy(WORKPATH.'thumbs/'.$id.'.'.preg_replace('/.*\
			unlink(WORKPATH.'thumbs/'.$id.'.'.preg_replace('/.*\
		}
